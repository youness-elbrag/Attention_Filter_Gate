from Layers import GlobalFilter , NormLayer , SoftmaxComplex
...
class AttentionFilter(nn.Module):
    ...
    def forward(self, g, x):
        ...
        atten = self.Softmax((g1_feq.transpose(3,2) @ x1_feq / torch.sqrt(2* torch.pi * norm_frwq)))
        inverse_atten = torch.fft.irfft2(atten, s=(H, W), dim=(2, 3), norm='ortho')   
        out = self.NormLayer(inverse_atten + x)
        return out, g1_feq , x1_feq
    
    def Varaince(self,x):
        x = x.var(-1 . unbiased = False ,keepdims= True)
#         y = torch.var(y,dim =(2,3),keepdims= True)
        return x 
